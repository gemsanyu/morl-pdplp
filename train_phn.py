...
@torch.no_grad()        
def validate_one_epoch(args, agent, phn, critic_phn, validation_nondom_list, critic_solution_list, validation_dataset, test_batch, tb_writer, epoch):
    agent.eval()
    ...
    validate_with_test(agent, phn, test_batch, tb_writer, epoch)
...
def validate_with_test(agent, phn, test_batch, tb_writer, epoch):
...
def run(args):
    ...
    for epoch in range(last_epoch+1, args.max_epoch):
        ...
        is_improving, validation_nondom_list, critic_solution_list, critic_phn = validate_one_epoch(args, agent, phn, critic_phn, validation_nondom_list, critic_solution_list, validation_dataset, test_batch, tb_writer, epoch)
        if not is_improving:

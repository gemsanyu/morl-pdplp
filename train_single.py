...
def run(args):
    ...
    comp_agent = torch.compile(agent)
    comp_best_agent = torch.compile(best_agent)
    for epoch in range(last_epoch+1, args.max_epoch):
        train_one_epoch(args, comp_agent, comp_best_agent, opt, train_dataset, tb_writer, epoch)
        best_validation_score, best_agent = validate_one_epoch(comp_agent, validation_dataset, best_validation_score, comp_best_agent, tb_writer, epoch)

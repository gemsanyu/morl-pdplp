...
from utils_moo import prepare_args, generate_params, get_ray_list
...
def run(args):
    agent, phn, critic_phn, _, _, _, _, _, test_batch, _, _ = setup_phn(args)
    ...
    with open(x_file_path.absolute(), "w") as x_file, open(y_file_path.absolute(), "w") as y_file, open(t_file_path.absolute(), "w") as t_file:
        test(agent,critic_phn,test_batch,x_file,y_file,t_file,args.num_ray)

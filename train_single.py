...
@torch.no_grad()
def validate_one_epoch(agent, validation_dataset, best_validation_score, best_agent, test_batch, tb_writer, epoch):
    agent.eval()
    ...
    tour_list, arrived_time_list, departure_time_list, travel_costs, late_penalties, reward_list, logprob_list, sum_entropies = solve_results
    tb_writer.add_scalar("Test Late Penalties", late_penalties.mean(), epoch)
    tb_writer.add_scalar("Test Travel Costs", travel_costs.mean(), epoch)

    score = late_penalties + travel_costs
    tb_writer.add_scalar("Test Score", score.mean(), epoch)
...
def run(args):
    max_patience = 10
    not_improving = 0
    agent, opt, best_agent_state_dict, best_validation_score, test_batch, tb_writer, last_epoch = setup(args)
    best_agent = copy.deepcopy(agent)
    ...
    for epoch in range(last_epoch+1, args.max_epoch):
        ...
        is_improving, best_validation_score, best_agent = validate_one_epoch(agent, validation_dataset, best_validation_score, best_agent, test_batch, tb_writer, epoch)

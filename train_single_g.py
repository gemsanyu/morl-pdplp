...
def train_one_epoch(args, agent, opt, train_dataset, tb_writer, epoch):
    ...
    for batch_idx, batch in tqdm(enumerate(train_dataloader), desc=f'Training epoch {epoch}'):
        ...
        G = np.flip(np.flip(reward_list, axis=1).cumsum(axis=1),axis=1)
        advantage_list = (G - G.mean(axis=1, keepdims=True))/G.std(axis=1, keepdims=True)
        advantage_list = np.sum(advantage_list, axis=-1)
        loss = logprob_list*torch.from_numpy(advantage_list).to(agent.device)
        loss = loss.sum(dim=-1).mean() - 0.05*sum_entropies.mean()
...
@torch.no_grad()
def validate_one_epoch(agent, validation_dataset, best_validation_score, best_agent_state_dict, tb_writer, epoch):
    agent.eval()
    ...
    if best_validation_score is None:
        ...
        best_agent_state_dict = copy.deepcopy(agent.state_dict())
        return best_validation_score, best_agent_state_dict
...
def run(args):
    ...
    for epoch in range(last_epoch+1, args.max_epoch):
        ...
        best_validation_score, best_agent_state_dict = validate_one_epoch(agent, validation_dataset, best_validation_score, best_agent_state_dict, tb_writer, epoch)
        save(agent, opt, best_validation_score, best_agent_state_dict, epoch, args.title)
